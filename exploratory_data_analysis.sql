...
SELECT 
  ...
  ROUND(
    SUM(Training_Cost)
  ) AS Failed_Training_Losses, 
  ROUND(
    SUM(
      CASE WHEN Training_Outcome = 'Failed' THEN 1 ELSE 0 END
    ) / COUNT(employee_id) * 100, 
    2
...
WHERE 
  t.Training_Outcome = 'Failed' 
ORDER BY 
  e.Current_Employee_Rating ASC;
...
FROM 
  training_and_development_data t 
...
FROM 
  employee_data 
WHERE 
  EmployeeStatus LIKE '%Terminated%' 
ORDER BY 
  Current_Employee_Rating ASC;
-- 4a. Business Units with most number of employees undergoing training
SELECT 
  COUNT(e.employee_id) AS total_employees_with_training, 
  e.BusinessUnit 
FROM 
  employee_data e 
  JOIN training_and_development_data t ON e.employee_id = t.employee_id 
GROUP BY 
  e.BusinessUnit 
ORDER BY 
  total_employees_with_training DESC;
...
SELECT 
  e.RaceDesc, 
  COUNT(e.RaceDesc) AS total_trainees 
FROM 
  employee_data e 
  JOIN training_and_development_data t ON e.employee_id = t.employee_id 
GROUP BY 
...
SELECT 
  e.RaceDesc, 
  COUNT(t.Training_Outcome) 
FROM 
  employee_data e 
  JOIN training_and_development_data t ON e.employee_id = t.employee_id 
WHERE 
  t.Training_Outcome = 'Completed' 
  OR t.Training_Outcome = 'Passed' 
GROUP BY 
  e.RaceDesc;
...
SELECT 
  ...
  COUNT(e.RaceDesc) AS total_trainees, 
  SUM(
    CASE WHEN Training_Outcome = 'Completed' 
    OR Training_Outcome = 'Passed' THEN 1 ELSE 0 END
  ) AS completed_trainees, 
  ROUND(
    (
      SUM(
        CASE WHEN Training_Outcome = 'Completed' 
